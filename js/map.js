...
async function initMainMap() {
  ...
  // Mövqeyə əsaslanaraq xəritəni yönəlt
  mainMap.locate({
    setView: true,
    maxZoom: 16,
...
function setupMapInteractions() {
  ...
  mainMap.on('locationfound', (e) => {
    const radius = Math.min(e.accuracy, 100);
...
.openOn(mainMap);
  ...
  mainMap.on('locationerror', (err) => {
    console.error("Location error:", err);
    alert("Mövqe təyin edilə bilmədi. Zəhmət olmasa brauzerdə icazə verin və yenidən cəhd edin.");
});
...
// kiçik xəritə — artıq statik koordinat yoxdur
